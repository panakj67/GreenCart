...
const ProductCard = ({product}) => {
    ...
    const {currency, addToCart, removeFromCart, cartItems} = useContext(AppDataContext);
            ...
            <div className="text-gray-500/60 text-sm">
                ...
                <div className="flex items-center gap-0.5">
                    {Array(5).fill('').map((_, i) => (
                            <img src={i < 4 ? assets.star_icon : assets.star_dull_icon} alt="" className="w-3 md:w-3.5" />
                    ))}
                    <p>(4)</p>
                </div>
                <div className="flex items-end justify-between mt-3">
                    ...
                    <div onClick={(e) => {e.stopPropagation}} className="text-[#4fbf8b]">
                        {!cartItems[product._id] ? (
                            <button className="flex items-center justify-center gap-1 bg-green-100 border cursor-pointer border-[#4fbf8b] md:w-[80px] w-[64px] h-[34px] rounded text-[#4fbf8b] font-medium" onClick={() => addToCart(product._id)} >
